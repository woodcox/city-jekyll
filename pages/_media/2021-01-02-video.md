...
<div class="flex-row">
  ...
  <div class="flex-tiny ty-one-third">
    <button class="muted-button" id="play-video-{{ mp4.guid | remove: "yt:video:" }}">
      <strong>Play:</strong> morning service on{{ mp4.title | remove: "City Evangelical Church" | date: "%-d %b %Y" }}</p>
    </button>
  </div>
</div>
<hr>
